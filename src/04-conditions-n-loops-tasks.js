...
function findFirstSingleChar(str) {
  ...
  return Object.keys(count).filter((n) => count[n] === 1)[0] || null;
}
...
function isCreditCardNumber(ccn) {
  if (ccn.toString().length % 2 === 0) {
    return ccn.toString().split('').reduce((acc, n, i) => {
      if (i % 2 === 0) {
        return +n * 2 > 9 ? acc + +n * 2 - 9 : acc + +n * 2;
      }
      return acc + +n;
    }, 0) % 10 === 0;
  }
...
function getCommonDirectoryPath(pathes) {
  const common = [];
  const arr = pathes.map((path) => path.replace(/\//g, '/ ').split(' '));
  const minArrLength = Math.min(...arr.map((elem) => elem.length));

  for (let i = 0; i < minArrLength; i += 1) {
    const firstElem = arr[0][i];

    if (arr.every((elem) => elem[i] === firstElem)) {
      common.push(firstElem);
    }
  }
  ...
  for (let i = 0; i < m1.length; i += 1) {
    ...
    for (let j = 0; j < m1.length; j += 1) {
      ...
      for (let k = 0; k < m1[i].length; k += 1) {
        arr2.push(m1[i][k] * m2[k][j]);
      }

      arr1.push(arr2.reduce((acc, n) => acc + n, 0));
    }
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < position.length; i += 1) {
    const elem = position[0][2];

    if (position[i][2 - i] !== elem) {
      break;
    }
